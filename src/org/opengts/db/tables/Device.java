    ...
    implements DataTransport
{
  ...
  //public static final String FLD_maintDescriptionHR0   = "maintDescriptionHR0";   // hours maint #0 description
    ...
    public static final DBField DataPushInfo[]           = {
        new DBField(FLD_lastELogState        , DTELogState.class   , DBField.TYPE_STRING(80)  , I18N.getString(Device.class,"Device.fld.lastELogState"        , "Last ELog state"             ), "edit=2"),
    ...
    **/
    public Map<String,Device.NearbyDevice> getNearbyDeviceMap(double radiusM)
        throws DBException
    {
        return this.getNearbyDeviceMap(radiusM, true/*active*/, null/*User*/);
    }
    ...
    **/
        ...
        throws DBException
    {
        ...
        return Device.GetNearbyDeviceMap(
            ...
            lastGP, radiusM, activeOnly, userAuth, 
            true/*sort*/);
    }
    ...
    **/
    public static Map<String,NearbyDevice> GetNearbyDeviceMap(
        Account account, String targetDevID, boolean inclTargetID,
        ...
        throws DBException
    {
        ...
        if (radiusM < 0.0) {
            Print.logError("Specified Radius is invalid: " + radiusM + " meters");
            return null;
        }
        ...
        //  (isActive != 0) and
        //  (lastValidLatitude  <= maxLat) and
        //  (lastValidLatitude  >= minLat) and
        //  (lastValidLongitude <= maxLon) and
        //  (lastValidLongitude >= minLon)
    ...
    **/
    public java.util.List<GeozoneTransition> checkGeozoneTransitions(long eventTime, GeoPoint eventGP)
    {
        ...
        // -    double devRadM = this.getDeviceProximityRadiusMeters();
        // -    User currUser = CURRENT_USER; // TODO: get current user
        // -    Map<String,Device.NearbyDevice> nbMap = this.getNearbyDeviceMap(devRadM, true/*active*/, currUser);
        // -    Collection<GeozoneArrived> gaList = GeozoneArrived.getArrivedList(deviceID);
        // -    // -- check for departs
        // -    for (GeozoneArrived ga : gaList) {
        // -        String gaDevID = gs.getGeozoneID(); // TODO: as device
        // -        Device.NearbyDevice nb = (nbMap != null)? nbMap.get(gaDevID) : null;
        // -        if (nb != null) {
        // -            // - still near this device
        // -            nbMap.remove(gaDevID);
        // -        } else {
        // -            // - departed this device
        // -            GeozoneArrived.removeArrivedDeviceID(this,gaDevID);
        // -            // TODO: add DeviceTransition: DeviceDepart
        // -        }
        // -    }
        // -    // -- check for arrives
        ...
        // -            // TODO: add DeviceTransition: DeviceArrive
        // -            GeozoneArrived.addArrivedDeviceID(this,nbDevID);
        // -        }
        // -    }
        ...
        if ((prevZone != null) && (thisZone == null)) {
            ...
            if (isDepart) {
                ...
                geoTrans.add(new GeozoneTransition(eventTime - 2L, sc, prevZone));
            }
            return geoTrans;
        }
        ...
        if ((prevZone == null) && (thisZone != null)) {
            ...
            if (isArrive) {
                ...
                geoTrans.add(new GeozoneTransition(eventTime - 1L, sc, thisZone));
            }
            return geoTrans;
        }
        ...
        // ---------------------------------------
        // -- no Geozone transitions

        /* no geozone transition */
        return null;
    ...
    *** RTP:"lastTS=123456789 lastSC=0xF010 isDriving=true|false"
    ...
    *** @param v The last HOS pata push time
    **/
        ...
        if (RTConfig.hasProperty(ARG_FIND_NEARBY)) {
            ...
            try {
                if (deviceExists && !GeoPoint.isValid(gp)) {
                    ...
                    Print.sysPrintln("Check Device location: " + deviceRcd.getLastValidLocation() + " RadiusM=" + radM);
                    nbMap = deviceRcd.getNearbyDeviceMap(radM, true, null/*User*/);
                } else {
                    ...
                    nbMap = Device.GetNearbyDeviceMap(
                        acct, null/*targetDevID*/, true,
                        gp, radM, true, null/*User*/,
                        true/*sort*/);
                }
            } catch (DBException dbe) {
