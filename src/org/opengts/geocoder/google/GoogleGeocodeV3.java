    ...
    */
    ...
    public ReverseGeocode getAddressReverseGeocode(GeoPoint gp, String localeStr, boolean cache)
    {
        ...
        if (!StringTools.isBlank(address)) { // status.equalsIgnoreCase(STATUS_OK)
            ...
            if (strAddr != null) { rg.setStreetAddress(strAddr); }
            if (city    != null) { rg.setCity(city);             }
          //if (county  != null) { rg.setCounty(county);         }
            if (state   != null) { rg.setStateProvince(state);   }
            if (postal  != null) { rg.setPostalCode(postal);     }
            if (country != null) { rg.setCountryCode(country);   }
            return rg;
        } else
        if (status.equalsIgnoreCase(STATUS_OK)          || 
            status.equalsIgnoreCase(STATUS_ZERO_RESULTS)  ) {
